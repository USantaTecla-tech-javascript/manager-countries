...
let answer = findNamesByRegionPopulation(getCountries(), region, population);
...
region = `Americas`;
population = 1000000;
answer = findNamesByRegionPopulation(getCountries(), region, population);
console.writeln(`¿Cuál es el nombre de los paises de ${region} con población superior o igual a ${population}?:
${answer !== `` ? answer : `No existen`}`);

let subregion = `Northern Europe`;
let area = 100000;
answer = findNamesBySubregionArea(getCountries(), subregion, area);
...
function findNamesByRegionPopulation(countries, region, population){
  let answer = ``;
...
function findNamesBySubregionArea(countries, subregion, area){
  let answer = ``;
  for (let i = 0; i < countries.length; i++) {
