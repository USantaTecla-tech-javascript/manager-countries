...
let name = `Spain`;
let indexOfByName = -1;
for (let i=0; i<COUNTRIES.length && indexOfByName === -1; i++) {
  if (COUNTRIES[i].name === name) {
    indexOfByName = i;
  }
}
console.writeln(`Posición de ${name} por nombre: ${indexOfByName}`);
...
let capital = `Madrid`;
let indexOfByCapital = -1;
for (let i=0; i<COUNTRIES.length && indexOfByCapital === -1; i++) {
  if (COUNTRIES[i].capital === capital) {
    indexOfByCapital = i;
  }
}
console.writeln(`Posición de ${capital} por capital: ${indexOfByCapital}`);
